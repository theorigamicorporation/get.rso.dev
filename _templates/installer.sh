...
usage() {
    cat <<'USAGE'
Usage: get-TOOLNAME.sh [OPTIONS]

Install TOOLNAME across Linux distributions with automatic distro detection.
...
"
    ...
    if [ "$_DISTRO_FAMILY" = "rhel" ] || [ "$_DISTRO_FAMILY" = "amazon" ]; then
        if command -v dnf >/dev/null 2>&1; then
            _count=$(( _count + 1 ))
            _AVAILABLE_METHODS="${_AVAILABLE_METHODS}${_count}:dnf:Install via dnf (native package manager)
"
        elif command -v yum >/dev/null 2>&1; then
            _count=$(( _count + 1 ))
            _AVAILABLE_METHODS="${_AVAILABLE_METHODS}${_count}:yum:Install via yum (native package manager)
"
        fi
    fi
...
"
    ...
    # GitHub release binary (always last)
...
# Install Methods
###########################
# TODO: Implement the install methods relevant to your tool.
...
install_via_flatpak() {
    ...
    $_SUDO_CMD flatpak install -y flathub "$TOOL_NAME" 2>/dev/null || {
        log "Could not find $TOOL_NAME on flathub." "ERR"
        exit 1
    }
}
...
install_via_github_release() {
    ...
    # TODO: Set the correct asset name pattern for your tool
    _asset_name="${TOOL_NAME}-linux-${_ARCH}"
