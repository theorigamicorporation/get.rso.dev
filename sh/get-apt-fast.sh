...
usage() {
    cat <<'USAGE'
Usage: get-apt-fast.sh [OPTIONS]

Install apt-fast on Debian-based Linux distributions with automatic distro detection.
...
check_existing_install() {
    ...
    if [ "$OPT_UPDATE" = true ]; then
        log "Proceeding with update..." "INFO"
        return 0
    fi
...
detect_available_methods() {
    ...
    if [ "$_DISTRO_FAMILY" != "debian" ]; then
        log "apt-fast is only available on Debian-based distributions" "ERR"
        exit 1
    fi

    if command -v apt-get >/dev/null 2>&1; then
        _count=$(( _count + 1 ))
        _AVAILABLE_METHODS="${_AVAILABLE_METHODS}${_count}:apt:Install via apt (PPA: ppa:apt-fast/stable)
"
    fi

    if [ -z "$_AVAILABLE_METHODS" ]; then
        log "No install methods available. Install apt-get or a package manager." "ERR"
        exit 1
    fi
}
...
###########################
install_via_apt() {
    log "Installing $TOOL_NAME via apt (PPA: ppa:apt-fast/stable)..." "INFO"
    ensure_sudo
