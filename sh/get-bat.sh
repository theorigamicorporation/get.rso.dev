...
usage() {
    cat <<'USAGE'
Usage: get-bat.sh [OPTIONS]

Install bat across Linux distributions with automatic distro detection.
...
resolve_tool_cmd() {
    if command -v bat >/dev/null 2>&1; then TOOL_CMD="bat"
    elif command -v batcat >/dev/null 2>&1; then TOOL_CMD="batcat"
    fi
}
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="bat-v${_version_num}-x86_64-unknown-linux-gnu.tar.gz" ;;
        arm64) _asset="bat-v${_version_num}-aarch64-unknown-linux-gnu.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "bat" -type f | head -1)
