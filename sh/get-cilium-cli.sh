...
# =============================================================================
# @description CLI for installing and managing Cilium in Kubernetes
...
# @verify cilium version --client
# @prereqs curl|wget
# =============================================================================
SCRIPT_VERSION="0.1"
SCRIPT_NAME="GET CILIUM"
...
usage() {
    cat <<'USAGE'
Usage: get-cilium-cli.sh [OPTIONS]

Install cilium across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="cilium-linux-amd64.tar.gz" ;;
        arm64) _asset="cilium-linux-arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "cilium" -type f | head -1)
