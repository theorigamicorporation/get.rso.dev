...
usage() {
    cat <<'USAGE'
Usage: get-flux.sh [OPTIONS]

Install flux across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="flux_${_version_num}_linux_amd64.tar.gz" ;;
        arm64) _asset="flux_${_version_num}_linux_arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "flux" -type f | head -1)
