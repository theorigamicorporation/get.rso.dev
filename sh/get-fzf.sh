...
usage() {
    cat <<'USAGE'
Usage: get-fzf.sh [OPTIONS]

Install fzf across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="fzf-${_version_num}-linux_amd64.tar.gz" ;;
        arm64) _asset="fzf-${_version_num}-linux_arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "fzf" -type f | head -1)
