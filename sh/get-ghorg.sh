...
usage() {
    cat <<'USAGE'
Usage: get-ghorg.sh [OPTIONS]

Install ghorg across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="ghorg_${_version_num}_Linux_x86_64.tar.gz" ;;
        arm64) _asset="ghorg_${_version_num}_Linux_arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "ghorg" -type f | head -1)
