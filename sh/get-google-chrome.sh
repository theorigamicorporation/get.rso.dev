...
usage() {
    cat <<'USAGE'
Usage: get-google-chrome.sh [OPTIONS]

Install Google Chrome across Linux distributions with automatic distro detection.
...
"
    ...
    if command -v flatpak >/dev/null 2>&1; then
        _count=$(( _count + 1 ))
        _AVAILABLE_METHODS="${_AVAILABLE_METHODS}${_count}:flatpak:Install via flatpak (Flathub)
"
    fi

    if [ -z "$_AVAILABLE_METHODS" ]; then
        log "No install methods available. Install apt, dnf, or flatpak." "ERR"
        exit 1
    fi
}
...
###########################
install_via_apt() {
    log "Installing $TOOL_NAME via apt (Google's official repo)..." "INFO"
    ensure_sudo
    ...
    # Add the repository
    log "Adding Google Chrome repository..." "INFO"
    printf 'deb [arch=amd64 signed-by=/usr/share/keyrings/google-chrome.gpg] https://dl.google.com/linux/chrome/deb/ stable main\n' \
        | $_SUDO_CMD tee /etc/apt/sources.list.d/google-chrome.list >/dev/null

    $_SUDO_CMD apt-get update -qq
    $_SUDO_CMD apt-get install -y -qq google-chrome-stable
}

install_via_dnf() {
    log "Installing $TOOL_NAME via dnf (Google's official repo)..." "INFO"
    ensure_sudo

    # Add the repository file
    log "Adding Google Chrome repository..." "INFO"
    $_SUDO_CMD tee /etc/yum.repos.d/google-chrome.repo >/dev/null <<'REPO'
[google-chrome]
name=google-chrome
...
REPO
    ...
    $_SUDO_CMD dnf install -y google-chrome-stable
}

install_via_flatpak() {
    log "Installing $TOOL_NAME via flatpak (Flathub)..." "INFO"
    flatpak install -y flathub com.google.Chrome 2>/dev/null || {
        log "Could not install Google Chrome from Flathub. Ensure Flathub is configured." "ERR"
        exit 1
    }
}
