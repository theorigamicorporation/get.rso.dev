...
usage() {
    cat <<'USAGE'
Usage: get-helm.sh [OPTIONS]

Install helm across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="helm-v${_version_num}-linux-amd64.tar.gz" ;;
        arm64) _asset="helm-v${_version_num}-linux-arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "helm" -type f | head -1)
