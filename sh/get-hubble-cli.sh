...
# =============================================================================
# @description CLI for Hubble network observability in Kubernetes
...
# @verify hubble version
# @prereqs curl|wget
# =============================================================================
SCRIPT_VERSION="0.1"
SCRIPT_NAME="GET HUBBLE"
...
usage() {
    cat <<'USAGE'
Usage: get-hubble-cli.sh [OPTIONS]

Install hubble across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="hubble-linux-amd64.tar.gz" ;;
        arm64) _asset="hubble-linux-arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "hubble" -type f | head -1)
