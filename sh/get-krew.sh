...
# @verify command -v kubectl-krew
# @prereqs curl|wget
# =============================================================================
SCRIPT_VERSION="0.1"
SCRIPT_NAME="GET KREW"

TOOL_NAME="krew"
TOOL_CMD="kubectl-krew"
GITHUB_REPO="kubernetes-sigs/krew"
...
usage() {
    cat <<'USAGE'
Usage: get-krew.sh [OPTIONS]

Install krew across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="krew-linux_amd64.tar.gz" ;;
        arm64) _asset="krew-linux_arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "krew-linux_amd64" -type f | head -1)
