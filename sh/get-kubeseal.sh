...
#shellcheck shell=sh
# =============================================================================
# get-kubeseal.sh — Install kubeseal across Linux distributions
# Usage: curl -sL get.rso.dev/sh/get-kubeseal | sh
#        sh get-kubeseal.sh [--method=github-release]
#        sh get-kubeseal.sh --interactive
#        sh get-kubeseal.sh --update
# =============================================================================
# @description CLI for Sealed Secrets encryption in Kubernetes
...
usage() {
    cat <<'USAGE'
Usage: get-kubeseal.sh [OPTIONS]

Install kubeseal across Linux distributions with automatic distro detection.
...
Examples:
  curl -sL get.rso.dev/sh/get-kubeseal | sh
  sh get-kubeseal.sh --method=github-release
  sh get-kubeseal.sh --interactive
  sh get-kubeseal.sh --update
USAGE
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="kubeseal-${_version_num}-linux-amd64.tar.gz" ;;
        arm64) _asset="kubeseal-${_version_num}-linux-arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "kubeseal" -type f | head -1)
