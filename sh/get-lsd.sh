...
usage() {
    cat <<'USAGE'
Usage: get-lsd.sh [OPTIONS]

Install lsd across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="lsd-v${_version_num}-x86_64-unknown-linux-gnu.tar.gz" ;;
        arm64) _asset="lsd-v${_version_num}-aarch64-unknown-linux-gnu.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "lsd" -type f | head -1)
