...
usage() {
    cat <<'USAGE'
Usage: get-micro.sh [OPTIONS]

Install micro across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="micro-${_version_num}-linux64.tar.gz" ;;
        arm64) _asset="micro-${_version_num}-linux-arm64.tar.gz" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "micro" -type f | head -1)
