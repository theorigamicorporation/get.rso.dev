...
# =============================================================================
# @description Build automated machine images for multiple platforms
...
usage() {
    cat <<'USAGE'
Usage: get-packer.sh [OPTIONS]

Install packer across Linux distributions with automatic distro detection.
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="packer_${_version_num}_linux_amd64.zip" ;;
        arm64) _asset="packer_${_version_num}_linux_arm64.zip" ;;
        *)     log "Unsupported arch for github-release: $_ARCH" "ERR"; exit 1 ;;
    esac

    _download_url="https://releases.hashicorp.com/packer/${_version_num}/${_asset}"
