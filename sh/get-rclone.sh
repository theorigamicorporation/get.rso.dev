...
#shellcheck shell=sh
# =============================================================================
# get-rclone.sh — Install rclone across Linux distributions
# Usage: curl -sL get.rso.dev/sh/get-rclone | sh
#        sh get-rclone.sh [--method=apt, dnf, yum, github-release]
#        sh get-rclone.sh --interactive
#        sh get-rclone.sh --update
# =============================================================================
...
usage() {
    cat <<'USAGE'
Usage: get-rclone.sh [OPTIONS]

Install rclone across Linux distributions with automatic distro detection.
...
Examples:
  curl -sL get.rso.dev/sh/get-rclone | sh
  sh get-rclone.sh --method=github-release
  sh get-rclone.sh --interactive
  sh get-rclone.sh --update
USAGE
}
...
install_via_github_release() {
    ...
    case "$_ARCH" in
        amd64) _asset="rclone-v${_version_num}-linux-amd64.zip" ;;
        arm64) _asset="rclone-v${_version_num}-linux-arm64.zip" ;;
    ...
    _binary=$(find "$_tmp_dir" -name "rclone" -type f | head -1)
